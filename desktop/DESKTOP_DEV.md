...
- [x] 桌面悬浮歌词（`Ctrl+Alt+D`，可拖拽，透明背景，逐行高亮 + 翻译/罗马音）
...
`Ctrl+Alt+D` 用于全局开关歌词窗，`Ctrl+Alt+L` 用于全局切换锁定状态；两者都由 Rust 全局快捷键插件直接处理，
不依赖主窗口 WebView。不使用 `Ctrl+D`，避免与 Excel 等应用的常用快捷键冲突。
前端不得调用 `unregisterAll()`，也不得再注册页面内 `Ctrl+Alt+D` fallback，否则会造成启动后失效或一次按键切换两次。
