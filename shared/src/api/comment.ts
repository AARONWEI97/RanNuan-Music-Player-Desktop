...
export const getMusicComment = (params: {
  id: number | string; limit?: number; offset?: number; before?: number;
...
export const getHotComment = (params: {
  id: number | string; type: number; limit?: number; offset?: number;
}) => request.get('/comment/hot', { params });

// ── 楼层评论（回复列表）──
export const getFloorComment = (params: {
  parentCommentId: number; id: number | string; type: number; limit?: number;
...
export const likeComment = (params: {
  id: number | string; cid: number; t: 1 | 0; type: number;
